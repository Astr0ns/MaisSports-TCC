...
router.post("/alterType", verificarAutenticacao, async function (req, res) {
    ...
    try {
        const emailExist = await connection.query(
            "SELECT id_empresa FROM empresas WHERE email = $1 AND cnpj = $2",
            [email, cnpj]
        );

        await connection.query(
            "INSERT INTO empresas (cnpj, email, senha, tipo) VALUES ($1, $2, $3, 'empresa')",
            [cnpj, email, hash]
        );
...
router.post('/delCEP', async function (req, res) {
    ...
    try {
        const result = await connection.query(
            "UPDATE usuario_clientes SET cep = '00000000' WHERE email = $1",
            [email]
        );
